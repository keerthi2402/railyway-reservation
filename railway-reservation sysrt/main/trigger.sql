...
BEGIN
    -- Check if the trigger operation is an INSERT
    ...
    -- Check if the trigger operation is an UPDATE and the booking is canceled
    ELSIF TG_OP = 'UPDATE' AND NEW.booking_status = 'Cancelled' THEN
        -- Loop through all passengers in the waiting queue in order of booking time
...
-- Create trigger to execute the seat allocation function after INSERT or UPDATE on the ticket table
